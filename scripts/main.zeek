...
event zeek_init() &priority=5
	{
    	Log::create_stream(TELNET::LOG, [$columns=TELNET::Info, $ev=log_telnet,
	$path="telnet"]);
    	Analyzer::register_for_ports(Analyzer::ANALYZER_TELNET, ports);
	}
event login_display(c: connection, display: string)
	{
	local rec: TELNET::Info = [$ts=network_time(), $id=c$id, $uid=c$uid,
	$display=display];
	c$telnet = rec;
...
event login_failure(c: connection, user: string, client_user: string, password: string, line: string)
	{
	local login_failure_rec: TELNET::Info = [$ts=network_time(),
		$id=c$id,$uid=c$uid,  $user=user, $client_user=client_user,
		$password=password, $line=line];
...
event login_success(c: connection, user: string, client_user: string, password: string, line: string)
	{
	local login_success_rec: TELNET::Info = [$ts=network_time(),
	$id=c$id,$uid=c$uid,  $user=user, $client_user=client_user,
	$password=password, $line=line];
...
event login_input_line(c: connection, line: string)
	{
	local login_input_line_rec: TELNET::Info = [$ts=network_time(),
	$id=c$id,$uid=c$uid,  $line=line];
	c$telnet = login_input_line_rec;
	Log::write(TELNET::LOG, login_input_line_rec);
	}
event login_output_line(c: connection, line: string)
	{
	local login_output_line_rec: TELNET::Info = [$ts=network_time(),
		$id=c$id,$uid=c$uid,  $line=line];
